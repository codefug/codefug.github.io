...
위의 시안을 분석해보고 Next.js 식으로 최대한 서버 컴포넌트를 이용하려면 다음과 같이 분기를 나눠서 생각해볼 수 있습니다.
...
브라우저 > 프론트엔드 서버(node.js 환경) > 백엔드 서버 순서입니다. 위처럼 움직이기 때문에 SSR이나 SSG, ISG 등등이 가능하게 된 것입니다.

서버 컴포넌트는 프론트엔드 "서버" 컴포넌트를 말합니다. 데이터를 분할해서 브라우저로 보낼 수 있는 streaming이 가능한 이유도 이것 때문입니다.

![](/images/2024-07-21/Pasted%20image%2020240722125217.png)

Suspense를 이용하면 위 사진처럼 fallback을 먼저 보내고 프론트엔드 서버에서 서버 컴포넌트 렌더링을 끝내면 브라우저에 컴포넌트를 보내고 fallback을 치우는 방식입니다.
...
쿠키는 통신에 담기기 때문에 다음의 사진대로 움직이게 됩니다.
...
Suspense 사용을 위한 서버 컴포넌트 설계를 위해 쿠키를 활용했다는 것을 알아두고 자세한 이야기는 최근 본 공고 설계 부분에서 다루도록 하겠습니다.
...
전역 "상태" 에 집중해보았습니다. zustand 개발자분은 깃허브 이슈에서 서버에는 "상태"가 존재하지 않다는 사실을 알면 이해가 편할 것입니다라고 이야기를 자주했습니다. 거기에 팀원이 사실 로컬 스토리지가 있는데 전역 상태와의 차이가 없는 것 같다고 말씀 해주셨습니다. 더구나 주스탄드 역시 rehydrate를 클라이언트 단에서 사용하고 로컬 스토리지를 활용하기 때문에 클라이언트 컴포넌트에서밖에 사용할 수 없었죠.
...
> 이 문제들을 해결하는 과정에서 기술은 어디까지나 도구라는 생각을 강하게 갖게 되었습니다. 전역 상태 관리라는 것의 의미를 많이 고민하고 설계를 진행했더라면 zustand라는 도구를 사용하지 않았을 것입니다.

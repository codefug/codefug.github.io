...
JavaScript 엔진은 힙(Heap) 메모리에 객체를 저장하고, 스택(Stack)에는 그 주소만 저장합니다.
...
객체 내부의 원시형 값(`name`, `age`)은 객체 메모리 공간 안에 직접 저장됩니다.
...
객체 안에 또 다른 객체가 있으면, 내부 객체도 별도의 메모리 주소를 가집니다.
...
flowchart TB
    ...
    subgraph 참조형["참조형 (Reference)"]
        ...
        Note2["주소를 저장<br/>obj1 === obj2 → false"]
    end
...
**핵심 차이점:**
- **원시형**: 값 자체가 스택에 저장되고, 비교 시 값을 비교
- **참조형**: 힙에 객체가 저장되고, 스택에는 주소만 저장. 비교 시 주소를 비교

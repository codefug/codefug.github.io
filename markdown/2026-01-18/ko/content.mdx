...
function SearchPage() {
  ...
  useEffect(() => {
    if (keyword) {
      inlineSaveDraft();
      setSearchCount((prev) => prev + 1); // 검색 횟수 추적
    }
  }, [inlineSaveDraft, keyword]); // 무한 루프 발생!
}
```
...
// 6단계: 리렌더링으로 inlineSaveDraft 함수 또 새로 생성
function SearchPage() {
  const inlineSaveDraft = () => {
    console.log("검색 실행:", keyword);
...
// 9단계: 리렌더링으로 inlineSaveDraft 함수 또 새로 생성
...
- `keyword`는 변경되지 않았지만 (`'react'`로 동일)
- `inlineSaveDraft` 함수만 계속 새로 생성됨 (메모리 주소만 변경: @100 → @200 → @300)
...
function SearchPage() {
  ...
  const saveDraft = useCallback(() => {
    console.log("검색 실행:", keyword);
  }, [keyword]); // keyword가 '' → 'react'로 변경되었으므로 새 함수 생성
  // @100 생성 (keyword='react'에 의존)
...
// 8단계: 사용자가 input에 "vue" 입력
// keyword: 'react' → 'vue'

// 9단계: keyword 변경으로 리렌더링
function SearchPage() {
  const saveDraft = useCallback(() => {
    console.log("검색 실행:", keyword);
  }, [keyword]); // keyword='vue'로 변경되었으므로 새 함수 생성
  // @200 생성 (keyword='vue'에 의존)

  // 10단계: useEffect deps 비교
  useEffect(() => {
    if (keyword) {
      saveDraft(); // 실행
      setSearchCount((prev) => prev + 1);
...
flowchart LR
    ...
    I2 --> J2["keyword 변경 시에만"]
    J2 --> K2["saveDraft @200 생성<br/>(새 keyword 의존)"]
    K2 --> L2["effect 실행"]
...
- `keyword` 변경으로 리렌더링되면 `inlineSaveDraft` 함수가 새로 생성됩니다 (새 주소)
- useEffect의 deps에 `inlineSaveDraft`가 있으면 주소 변경을 감지해 effect가 실행됩니다
- effect 내부에서 `refetch()`를 호출하면 React Query 상태가 변경되어 리렌더링이 발생합니다
- 리렌더링으로 `inlineSaveDraft`가 또 새로 생성되어 무한 루프가 발생합니다

#### 1단계: 초기 입력으로 인한 리렌더링

사용자가 input 필드에 "react"를 입력하는 순간부터 시작됩니다.

```jsx
// 사용자가 input에 "react" 입력
<input value={keyword} onChange={(e) => setKeyword(e.target.value)} />
// → setKeyword('react') 호출
```
...
// 2. 컴포넌트 리렌더링 발생
function SearchPage() {
  // 3. 매 렌더링마다 새로운 함수 생성 (새 메모리 주소)
  const inlineSaveDraft = () => {
    console.log("검색 실행:", keyword); // @200 (이전 렌더링에서는 @100)
    refetch();
  };
...
flowchart LR
    A["사용자가 input에 입력"] --> B["keyword 상태 변경"]
    B --> C["컴포넌트 리렌더링"]
    C --> D["inlineSaveDraft 새로 생성<br/>(@100 → @200)"]
    D --> E["useEffect deps 비교<br/>(@100 !== @200)"]
    E --> F["effect 실행"]
...
**핵심 문제:** `inlineSaveDraft` 함수가 매 렌더링마다 새로운 메모리 주소를 받기 때문에, React는 이를 "변경된 값"으로 인식합니다.

#### 2단계: React Query 상태 변경으로 인한 연쇄 반응

1단계에서 effect가 실행되면, 내부에서 `refetch()`가 호출됩니다.

```jsx
useEffect(() => {
  if (keyword) {
    inlineSaveDraft(); // 이 함수 내부에서 refetch() 호출
  }
}, [inlineSaveDraft, keyword]);
...
// 2. refetch() 호출로 React Query 상태 변경
const { data, refetch, isFetching } = useQuery({
  queryKey: ["search", keyword],
  queryFn: () => fetchSearchResults(keyword),
  enabled: !!keyword,
...
// data: undefined → [...results] (데이터 업데이트)

// 3. React Query 상태 변경으로 컴포넌트 리렌더링 발생
// → SearchPage() 함수 전체가 다시 실행됨

// 4. 리렌더링으로 inlineSaveDraft 함수가 또 새로 생성
function SearchPage() {
  const inlineSaveDraft = () => {
    // @300 (이전에는 @200)
    console.log("검색 실행:", keyword);
...
```mermaid
flowchart LR
    F["effect 실행"] --> G["refetch() 호출"]
    G --> H["React Query 상태 변경<br/>(isFetching: true → false)"]
    H --> I["리렌더링 발생"]
    I --> J["inlineSaveDraft 또 새로 생성<br/>(@200 → @300)"]
    J --> K["useEffect deps 비교<br/>(@200 !== @300)"]
    K --> F
...
**핵심 문제:** `refetch()`가 React Query 상태를 변경하고, 이로 인해 컴포넌트가 리렌더링되면서 `inlineSaveDraft` 함수가 또 새로 생성됩니다. 개발자는 "keyword가 변경되어서 effect가 실행됐겠지"라고 생각하지만, 실제로는 React Query 상태 변경 → 함수 재생성 → effect 재실행의 연쇄 반응입니다.

#### 3단계: 무한 루프 발생

2단계에서 effect가 다시 실행되면, 같은 과정이 반복됩니다.

```jsx
// 2단계에서 effect 재실행
useEffect(() => {
  if (keyword) {
    inlineSaveDraft(); // @300 주소의 함수 호출
  }
...
const inlineSaveDraft = () => {
  console.log("검색 실행:", keyword);
  refetch(); // React Query 상태 변경
};

// React Query 상태 변경 → 리렌더링
// → inlineSaveDraft @400 생성
// → useEffect deps 비교: @300 !== @400
// → effect 또 실행
// → refetch() 또 호출
// → 리렌더링
// → inlineSaveDraft @500 생성
// → ... 무한 반복 🔄
```
...
- Components 탭에서 컴포넌트가 계속 깜빡임 (리렌더링)
- Profiler에서 렌더링이 무한 반복되는 것을 확인 가능

```mermaid
flowchart LR
    K["useEffect deps 비교<br/>(@200 !== @300)"] --> F["effect 실행"]
    F --> G["refetch() 호출"]
    G --> H["React Query 상태 변경"]
    H --> I["리렌더링 발생"]
    I --> J["inlineSaveDraft 또 새로 생성<br/>(@300 → @400)"]
    J --> K2["useEffect deps 비교<br/>(@300 !== @400)"]
    K2 --> F
    F --> L["무한 루프! 🔄"]
...
1. `keyword`는 변경되지 않았지만 (`'react'`로 동일)
2. `inlineSaveDraft` 함수만 계속 새로 생성됨 (메모리 주소만 변경)
3. React는 deps 비교 시 메모리 주소를 비교하므로 (`@200 !== @300 !== @400`)
4. 매번 "변경됨"으로 판단하여 effect를 실행
5. effect 실행 → refetch → 리렌더링 → 함수 재생성 → effect 실행 (무한 반복)

**왜 추적이 어려운가?**

1. 사용자가 input에 값을 입력하면 `keyword` 상태가 변경됩니다
2. `keyword` 변경으로 리렌더링되면 `inlineSaveDraft` 함수가 새로 생성됩니다 (새 메모리 주소)
3. useEffect의 deps에 `inlineSaveDraft`가 있으면 주소 변경을 감지해 effect가 실행됩니다
4. effect 내부에서 `refetch()`를 호출하면 React Query의 쿼리 상태(`isFetching`, `data` 등)가 변경됩니다
5. 쿼리 상태 변경으로 컴포넌트가 또 리렌더링되고, `inlineSaveDraft`가 또 새로 생성됩니다
6. 개발자는 "keyword가 변경되어서 effect가 실행됐겠지"라고 생각하지만, 실제로는 React Query의 상태 변경 → 함수 재생성 → effect 재실행의 연쇄 반응이 발생합니다

...
Primitives store the value itself in the variable.
...
`a` and `b` each store `1` in independent memory spaces. The comparison operator (`===`) compares the stored values themselves, so it returns `true`.

## Reference Data

Reference types store a memory address in the variable.
...
`obj1` and `obj2` have the same content but point to different memory addresses. The comparison operator (`===`) compares addresses, so it returns `false`.
...
The JavaScript engine stores the object in heap memory and only stores the address in the stack.
...
flowchart TB
    ...
    subgraph Heap["Heap"]
        direction TB
        O1["@1000<br/>{ name: 'Kim', age: 25 }"]
    end

    V1 --> O1
```

**Step 3: Primitive Values Inside Objects**

Primitive values inside objects (`name`, `age`) are stored directly within the object's memory space.
...
When an object contains another object, the inner object also has its own separate memory address.
...
flowchart TB
    subgraph Primitive["Primitive"]
        ...
        Note1["Stores value itself<br/>a === b → true"]
    end
    ...
    subgraph Reference["Reference"]
        ...
        Note2["Stores address<br/>obj1 === obj2 → false"]
    end
...
**Key Differences:**
- **Primitive**: The value itself is stored in the stack, and values are compared during comparison
- **Reference**: The object is stored in the heap, and only the address is stored in the stack. Addresses are compared during comparison

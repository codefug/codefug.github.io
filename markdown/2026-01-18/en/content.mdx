...
flowchart TB
    subgraph Variables["Variables"]
        direction TB
        V1["Variable obj1 → @1000"]
        V2["Variable obj2 → @2000"]
    end
...
```mermaid
flowchart TD
    A["Component renders"] --> B["inlineSaveDraft function created (new address)"]
    B --> C["useEffect deps comparison"]
    C --> D{"inlineSaveDraft address different?"}
    D -->|Yes| E["effect runs: inlineSaveDraft()"]
    E --> F["setSearchCount called"]
    F --> G["State change → re-render"]
    G --> A
    D -->|No| H["effect skipped"]
```
...
```mermaid
flowchart TD
    subgraph "Without useCallback (inlineSaveDraft)"
        A1["keyword='react' typed"] --> B1["inlineSaveDraft @100"]
        B1 --> E1["effect runs, searchCount=1"]
        E1 --> A2["Re-render (searchCount changed)"]
        A2 --> B2["inlineSaveDraft @200 (new address)"]
        B2 --> E2["effect runs again, searchCount=2"]
        E2 --> A3["Re-render..."]
        A3 --> B3["Infinite loop!"]
    end

    subgraph "With useCallback (saveDraft)"
        C1["keyword='react' typed"] --> D1["saveDraft @100"]
        D1 --> F1["effect runs, searchCount=1"]
        F1 --> C2["Re-render (searchCount changed)"]
        C2 --> D1
        D1 --> F2["effect skipped (same address)"]
        F2 --> C3["Only when keyword changes"]
        C3 --> D2["saveDraft @200 (new address)"]
        D2 --> F3["effect runs, searchCount=2"]
    end
```
...
```mermaid
flowchart TD
    subgraph "Problem with React Query"
        A["User types 'react'"] --> B["keyword changes → re-render"]
        B --> C["inlineSaveDraft recreated (@200)"]
        C --> D["useEffect runs"]
        D --> E["refetch() called"]
        E --> F["React Query state changes<br/>(isFetching: true → false)"]
        F --> G["Re-render occurs"]
        G --> H["inlineSaveDraft recreated again (@300)"]
        H --> I["useEffect runs again"]
        I --> J["refetch() called again"]
        J --> K["Infinite loop!"]
    end
```

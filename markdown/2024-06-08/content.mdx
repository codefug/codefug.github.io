...
다시 말해 위의 에러는 "처음 서버에서 준 HTML이랑 다음에 받은 UI가 다르다는 것이다." 라는 거다.
...
NEXT*PUBLIC*~ 로 env에 넣어놓으면 빌드 타임에 js bundle에 환경 변수를 넣어놓음으로써 브라우저에서 접근할 수 있게 된다.
...
> [!note] 2025-04-21 추가 보완 자료
>
> NEXT_PUBLIC으로 지정하게 되면 빌드 타임에 환경 변수를 넣는다고 되어있는데 더 자세히 이야기 하면 빌드 파일 내부에 해당 변수를 넣겠다는 뜻이다.
> 그래서 정말 중요한 환경 변수의 경우 NEXT_PUBLIC으로 사용하면 안된다.
>
> 이때 프론트엔드 서버의 환경 변수를 사용하면 해결할 수 있는데 그것이 위에 내가 잘못 사용했던 process.env.~ 의 정체이다.
> 서버 컴포넌트나 서버 사이드 렌더링 ( getServerSideProps ), 라우트 핸들러 등 프론트엔드 서버 단에서 사용되는 함수들은 해당 환경 변수에 접근할 수 있다.

...
`SearchParamsContext` is a React Context that the router injects with the current URL's parsed query parameters. In the browser, `useSearchParams` is just synchronous code that reads from that context.
...
The reason `useSearchParams` demands a Suspense isn't because of async data like in browser circumstance — it's because it throws `BailoutToCSRError` during prerender and needs a boundary to catch the error. And that boundary is exactly the scope that will be deferred to CSR.
...
To wrap up: when using `useSearchParams`, treat it as the act of explicitly choosing the CSR scope. That's how you preserve as much of the static-generation benefit as possible. The components that use `useSearchParams` must be wrapped in a Suspense, and that Suspense should be as narrow as possible.

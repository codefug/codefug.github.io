...
> [!NOTE] hydration
>
> React 공식 문서는 `hydrateRoot`를 "`react-dom/server`가 만든 HTML payload에 React를 붙여, DOM을 재생성하지 않고 인터랙티브하게 만드는 것"으로 정의합니다. 즉 hydration은 서버 HTML과 클라이언트 React 트리를 일치시키며 이벤트 핸들러 및 state를 연결하는 과정입니다.
>

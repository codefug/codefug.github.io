...
나의 상황에 딱 맞는 문서를 찾게 되어서 진행하게 되었다. 이제 많이 했기에 간단한 용어 정도는 알고 있음에 기뻐하며 말입니다.
...
위처럼 말입니다. 내 프로젝트를 식별하기 위해서 존재하니 어디에 적어두자.
...
이 파일은 vercel-action을 이용해서 vercel token과 github token에 맞는 배포를 합니다. branches를 통해 브랜치를 특정해놨는데 우리 팀은 main이 배포 브랜치이기 때문에 main에 push하는 순간 vercel에 배포할 수 있도록 설정했다.
...
preview의 경우에도 이와 같다. 이제 PR에서 commit될 때마다 최상단의 commit이 바뀌게 될 것입니다.

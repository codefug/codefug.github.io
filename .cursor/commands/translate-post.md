...
## 목적
한국어로 작성된 마크다운 블로그 포스트를 영어와 일본어로 자동 번역하여 다국어 블로그를 운영합니다.
...
3. **일본어 번역 생성**
   - content.mdx의 모든 한국어 텍스트를 자연스러운 일본어로 번역
   - frontmatter.mdx의 title, excerpt, category를 일본어로 번역
   - 코드 블록, 코드 주석은 그대로 유지
   - 기술 용어는 가타카나로 표기 (예: "리액트" → "リアクト")
   - `markdown/{post-id}/ja/content.mdx`에 저장
   - `markdown/{post-id}/ja/frontmatter.mdx`에 저장

## 번역 가이드라인

### 일반 원칙
- **자연스러운 번역**: 직역보다는 각 언어의 자연스러운 표현 사용
- **기술 용어**: 일관성 있게 처리
  - 영어: 원어 그대로 (React, TypeScript, Next.js)
  - 일본어: 카타카나 표기 (リアクト, タイプスクリプト, ネクストジェイエス)
...
リアクトフックは関数型コンポーネントで状態管理を可能にします。

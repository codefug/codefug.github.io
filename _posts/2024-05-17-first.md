...
그럼에도 만들게 된 이유는 4가지 정도 생각할 수 있을 것 같다.
1. 이전까지는 velog로 글을 작성했었는데 velog에서는 flex-direction: row 식으로 글이 보여졌다. 한눈에 보기가 너무 불편했다.

2. 평소 obsidian에 공부한 것들을 정리하고 그 공부한 것들을 정리해서 블로그에 올리곤 하는데 블로그에 올릴 때 파일을 하나씩 넣으면서 화났던 경험이 있다. 어짜피 같은 md니깐 기존의 글 방식에서 크게 벗어나지도 않지 않을까 싶었다.

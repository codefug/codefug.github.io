...
excerpt: startTransition, progress bar
toc: true
toc_sticky: true
categories:
  - kkom-kkom
...
### <mark style="background: #FFF3A3A6;">progress-bar</mark>
...
99에서 멈춰있는 것도 99%에서 로딩에 필요한 데이터가 많아서 멈춰있는게 아니라 그냥 시간이 다 된 것일 뿐이라는 걸 알았다.

startTransition을 이용한 동시성 렌더링으로 progress bar가 언마운트 되는 로직을 router.push와 함께 넣어둔 부분이 굉장히 인상 깊었다.

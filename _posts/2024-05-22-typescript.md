...
예전 코드나 바벨로 변환된 코드에서 자주 보이는 var라는 선언어가 굉장히 장난을 많이 친다. 
...
ts는 tsc라는 컴파일러를 이용해서 js로 변환되고 (ts를 온전히 돌릴 수 있는 deno, Bun 등의 환경이 있지만 아직 대중화되지 않았다.)
이후 변환된 js파일을 실행한다.

tsc는 두가지 역할을 하는데 하나는 이전에 말했던 `변환` 이고 다른 하나는 타입 검사이다.
(타입 검사에 통과 못해도 JS로 변환은 된다.)

## 실제 코드와 여기서 알게된 TS가 극복한 문제들
